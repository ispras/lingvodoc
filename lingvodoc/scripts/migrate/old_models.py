...
old_DBSession = scoped_session(sessionmaker(create_engine('postgresql+psycopg2://postgres@127.0.0.1:5433/lingvodoc')))
...
def find_by_translation_string_ui(locale_id, translation_string):
        trstr = old_DBSession.query(UITranslationString).\
...
def find_by_translation_string(locale_id, translation_string):
        trstr = old_DBSession.query(UserEntitiesTranslationString).\
            filter_by(locale_id=locale_id, translation_string=translation_string).first()
        if not trstr:
            trstr = old_DBSession.query(UserEntitiesTranslationString).\
                filter_by(locale_id=1, translation_string=translation_string).first()
...
def add_translation_to_translation_string_ui(locale_id, translation, translation_string):
        ...
        uets = old_DBSession.query(UITranslationString).filter_by(locale_id=locale_id,
...
def add_translation_to_translation_string(locale_id, translation, translation_string, client_id):
        ...
        client = old_DBSession.query(Client).filter_by(id=client_id).first()
        uets = old_DBSession.query(UserEntitiesTranslationString).filter_by(locale_id=locale_id,
                                                                        translation_string=translation_string).first()
        if not translation:
            translation = translation_string
        if not uets:
...
class Language(Base, TableNameMixin):
    ...
    def set_translation(self, request):
        ...
        if not translation:
            translation = translation_string
        if not uets:
            uets = UserEntitiesTranslationString(object_id=old_DBSession.query(UserEntitiesTranslationString).filter_by(client_id=client.id).count()+1,
                                                 client_id=client.id,
                                                 locale_id=locale_id,
                                                 translation_string=translation_string,
                                                 translation=translation)
