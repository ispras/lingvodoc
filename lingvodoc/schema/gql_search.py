...
def search_mechanism_boolean(dictionaries, category, state_gist_id, limited_gist_id, search_strings, publish, accept, adopted,
    ...
    for or_element in full_or_block:
        result_and_block = and_(or_element, *perspective_and_block)
        query = DBSession.query(dbLexicalEntry, dbDictionaryPerspective, dbDictionary).filter(result_and_block,
                                                  tuple_(dbLexicalEntry.client_id, dbLexicalEntry.object_id).in_(
                                                      lexes)).yield_per(yield_batch_count)
        all_results.append(query)

    resolved_search = all_results[0].union(*all_results[1:])
    # resolved_search = set()
    # for entity in chain.from_iterable(all_results):
    #     resolved_search.add(entity)
    ...
    res_lexical_entries = [graphene_obj(ent[0], LexicalEntry) for ent in resolved_search]
    res_perspectives = [graphene_obj(ent[1], DictionaryPerspective) for ent in resolved_search]
    res_dictionaries = [graphene_obj(ent[2], Dictionary) for ent in resolved_search]
