...
import ru.ispras.lingvodoc.frontend.app.model.{Language, Perspective, PerspectiveMeta, _}
...
class HomeController(scope: HomeScope, backend: BackendService, val timeout: Timeout, val exceptionHandler: ExceptionHandler)
  ...
  doAjax(() => {
    ...
    backend.allPerspectivesMeta flatMap { p =>
      ...
      backend.getPublishedDictionaries flatMap { languages =>
        backend.perspectives(published = true) flatMap { perspectives =>
