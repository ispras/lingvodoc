...
@js.native
trait PerspectiveStatisticsModalScope extends Scope
{
...
@injectable("PerspectiveStatisticsModalController")
class PerspectiveStatisticsModalController(
  scope: PerspectiveStatisticsModalScope,
  ...
  def gather(): Unit =
  {
    ...
    backend.perspectiveStatistics(perspectiveId,
        ...
        scope.statistics_data = js.Array(statistics_data .values .toSeq
          ...
          .map { user_data =>
            ...
            if (user_data.contains("entities"))
            {
              ...
              /* Grouping entity statistics by publishing state, organizing it for table display. */

              for (state <- sorted_keys_total_last(entity_data.keySet))
              {
                val state_data = entity_data(state).asInstanceOf[js.Dictionary[js.Dictionary[Int]]]
                var entity_table_group: js.Array[js.Array[String]] = js.Array()

                for ((entity_type, index) <-
                  sorted_keys_total_last(state_data.keySet) .zipWithIndex)
                {
                  val entity_data = state_data(entity_type)
                  val state_string = if (index <= 0) state else ""

                  /* Each entity type is a separate row. */

                  entity_table_group.append(js.Array(state_string, entity_type,
