...
trait PerspectivePropertiesScope extends Scope {
  ...
  var authors: UndefOr[String] = js.native
  var selectedFileId: UndefOr[String] = js.native
  var linkedFiles: js.Array[File] = js.native
  var location: Option[Location] = js.native
...
class PerspectivePropertiesController(scope: PerspectivePropertiesScope,
  ...
  private[this] var metadata = Option.empty[MetaData]
  ...
  scope.authors = Option.empty[String].orUndefined
  scope.selectedFileId = Option.empty[String].orUndefined
  scope.linkedFiles = js.Array[File]()
  scope.location = Option.empty[Location]
  ...
  @JSExport
  def getCurrentLocationString(): UndefOr[String] = {
    scope.location.map(loc => s"[${loc.location.lat.toString}, ${loc.location.lng.toString}]").orUndefined
  }

  @JSExport
  def editLocation(): Unit = {
    val options = ModalOptions()
    ...
    instance.result foreach { p =>
      scope.location = p.map(Location("location", _))
    }
  }
  ...
  @JSExport
  def addFile(): Unit = {
    scope.selectedFileId.toOption foreach { fileId =>
      scope.files.find(_.getId == fileId) foreach { file =>
        if (!scope.linkedFiles.exists(_.getId == file.getId)) {
          scope.linkedFiles = scope.linkedFiles :+ file
        }
      }
    }
  }
  ...
  def removeFile(file: File): Unit = {
    scope.linkedFiles = scope.linkedFiles.filterNot(_.getId == file.getId)
  }
  ...
  def ok(): Unit = {
    ...
    metadata foreach { meta =>
      ...
      // authors
      val authors = scope.authors.toOption.flatMap { a =>
        if (a.isEmpty)
          Option.empty[Authors]
        else
          Some(Authors("authors", a))
      }
      // location
      val location = scope.location

      // blobs
      val info = scope.linkedFiles.map(file => Blob("blob", CompositeId.fromObject(file))).toSeq

      val umeta = meta.copy(authors, location, info)

      backend.setPerspectiveMeta(CompositeId.fromObject(dictionary), CompositeId.fromObject(perspective), umeta).map { _ =>
          ...
          Future.sequence(reqs) flatMap { _ =>
            ...
            backend.fields() flatMap { f =>
              ...
              backend.getFields(CompositeId.fromObject(dictionary), CompositeId.fromObject(perspective)) flatMap { fields =>
                parsePerspective(perspective, fields).flatMap { layer =>
                  scope.layers.push(layer)

                  backend.userFiles flatMap { userFiles =>
                    //XXX: webui specific
