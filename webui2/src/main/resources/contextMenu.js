...
.directive('contextMenu', ["$parse", "$q", "CustomService", "$sce", function ($parse, $q, custom, $sce) {
    ...
    var renderContextMenu = function ($scope, event, options, model, level, customClass) {
        ...
        function removeAllContextMenus(e) {
            // WARN: original version was
            //if (!$(e.target).closest('.dropdown').length) {
            // , but it didn't dispatched events correctly, because context menu div
            // (which has angular-bootstrap-contextmenu class) occupies the whole screen.
