...
class CorporaController(scope: CorporaScope, modal: ModalService, location: Location, userService: UserService, backend: BackendService, val timeout: Timeout) extends
  AbstractController[CorporaScope](scope) with AngularExecutionContextProvider with LoadingPlaceholder {
